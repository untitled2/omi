...
// - no log levels
// - no log rotation
// - no queue, the log events will be at the mercy of OS process scheduler
// - not very efficient (esp. timestamp generation)
...
public:
	...
	{
		auto ret = std::async(std::launch::async, [this, &message]() {
...
public:
    ...
    void log(const std::string& message) override final
	{
		auto r = std::async(std::launch::async, [this, &message]() {
			std::lock_guard<std::mutex> lock(mutex);
			try {
